...
#include <fstream>   // Trabaja con archivos
...
// Función para limpiar línea (quitar espacios al inicio/final y comentarios)
...
void executeFromFile(const std::string& filename) {
    ...
    while (std::getline(file, line)) {
        ...
        if (result == "EXIT") {
            std::cout << "Comando 'exit' encontrado en línea " << lineNumber << "\n";
            break;
        }
...
// Función para ejecutar múltiples comandos desde string (separados por newline)
...
int main(int argc, char* argv[]) {
    ...
    if (argc > 1) {
        ...
        if (arg1 == "-h" || arg1 == "--help") {
            ...
            // Ejecutar comando(s) desde argumento
            std::cout << "C++ DISK\n";
            std::cout << "MIA Proyecto 1 - 2026\n\n";
