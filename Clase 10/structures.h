...
struct Partition {
    ...
    int part_correlative;      // Número correlativo de montaje
    char part_id[4];           // ID de montaje (ej: "341A")
...
struct FileBlock {
    char b_content[64];            // Contenido del archivo (64 bytes = block size)
...
struct PointerBlock {
    int b_pointers[16];            // Array de apuntadores a bloques (16 * 4 = 64 bytes)
